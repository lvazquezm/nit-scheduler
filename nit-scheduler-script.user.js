...
(function() {
    ...
    var report = []; //findTds
    var interviews = []; //fillInterviews | Calendario Outlook
    var shadows_date_time = [];
    var shadows_date_partial = [];
    var shadows_date = [];
    var eventList = [];
    ...
    function findMatches(){
        ...
        btn.onclick = function(){
            ...
            let tab = document.querySelector(".report-table");
            //tab.style.display = "none";

            report = [];
            interviews = [];

            let trs = document.querySelectorAll(".report-body > tr");
            ...
            findShadows();
            createShadowTable();
            createShadowTablePartial();
            createShadowTableDate()
            createNoShadowTable();
            ...
            console.log("Shadows date and time");
            console.log(shadows_date_time);

            console.log("Shadows date");
            console.log(shadows_date);*/
        }

        const btnParent = document.querySelector(".btn-matches");
        btnParent.insertBefore(btn, null);
    }

    function findTds(trss){
        let aux = trss.children;
        let obj = {
            id:"",
            type: "",
            workStep:"",
        ...
        $(aux).each(function(j){
            ...
            switch(j){
                case 0:
                    obj.id = valor;
                    obj.hireLink = "https://hire.amazon.com/interviews/"+valor+"#/interview_event";
                    break;
                case 1:
                    obj.type = valor;
                    break;
                ...
                case 7:
                    ...
                    break;
                case 8:
                    obj.jobLevel = parseInt(valor);
                    break;
                case 9:
                    obj.departmentId = valor;
                    break;
                case 10:
                    obj.interviewerLogin = valor;
                    break;
                case 11:
                    obj.interviewerName = valor;
                    break;
                case 12:
                    obj.debriefDate = valor;
                    break;
                case 13:
                    obj.country = valor;
                    break;
                case 14:
                    obj.recruiterLogin = valor;
                    break;
                case 15:
                    obj.sourcerLogin = valor;
                    break;
                case 16:
                    obj.rcGroup = valor;
                    break;
    ...
    function findShadows(){
        shadows_date_time = [];
        shadows_date_partial = [];
        shadows_date = [];

        let report_start_date = new Date();
        let report_end_date = new Date();

        let interview_start_date = new Date();
        let interview_end_date = new Date()

        for(let i=0; i<report.length; i++){
            for(let j=0; j<interviews.length; j++){

                report_start_date = new Date(report[i].date + "T"+ report[i].startTime);
                report_end_date = new Date(report[i].date + "T"+ report[i].endTime);

                interview_start_date = new Date(interviews[j].date+"T"+interviews[j].start);
                interview_end_date = new Date(interviews[j].date+"T"+interviews[j].end);

                /*console.log("Inicio loop");
                console.log(report_start_date);
                console.log(report_end_date);
                console.log(interview_start_date);
                console.log(interview_end_date);
                console.log("Fin loop");*/

                //console.log(report[i].date);
                //console.log(interviews[j].date);

                //console.log(report[i].date === interviews[j].date);

                let shadow = {
                        suggestedShadow: interviews[j].alias,
                        shadowLevel: interviews[j].level,
                        availabilityStart: interviews[j].start +" (UTC -06:00)",
                        availabilityEnd: interviews[j].end +" (UTC -06:00)",
                        type: report[i].type,
                        workStep: report[i].workStep,
                        date: report[i].date,
                        interviewStartTime: report[i].startTime +" (UTC -06:00)",
                        interviewEndTime: report[i].endTime +" (UTC -06:00)",
                        jobId: report[i].jobId,
                        jobTitle: report[i].jobTitle,
                        jobFamily: report[i].jobFamily,
                        jobLevel: report[i].jobLevel,
                        departmentId: report[i].departmentId,
                        interviewerLogin: report[i].interviewerLogin,
                        interviwerName: report[i].interviewerName,
                        debriefDate:report[i].debriefDate,
                        country: report[i].country,
                        recruiterLogin: report[i].recruiterLogin,
                        sourcerLogin: report[i].sourcerLogin,
                        //rcGroup:report[i].rcGroup,
                        hireLink:report[i].hireLink
                    }

                if(report_start_date >= interview_start_date && report_end_date <= interview_end_date && report[i].type === "Phone Screen" && report[i].jobLevel <= interviews[j].level){

                    report[i].shadow = true;
                    shadows_date_time.push(shadow);

                }else if(report[i].date == interviews[j].date && interview_end_date >= report_start_date && report[i].type === "Phone Screen" && report[i].jobLevel <= interviews[j].level){
         
                    report[i].shadow = true;
                    shadows_date_partial.push(shadow);
                }else if(report[i].date == interviews[j].date && report[i].type === "Phone Screen" && report[i].jobLevel <= interviews[j].level){
                    
                    report[i].shadow = true;
                    shadows_date.push(shadow);
                }
            }
        }
        ...
        if(shadows_date_time.length == 0){
            shadows_date_time.push(shadow_aux);
        }
        if(shadows_date_partial.length == 0){
            shadows_date_partial.push(shadow_aux);
        }
        if(shadows_date.length == 0){
            shadows_date.push(shadow_aux);
        }
    }
    ...
    function createShadowTable(){
        ...
        for (let element of shadows_date_time) {
            let row = shadowTable.insertRow();
            for (let key2 in element) {
                if(key2 == "workStep"){
                    let cell = row.insertCell();

                    let a = document.createElement('a');
                    let linkText = document.createTextNode(element[key2]);
                    a.appendChild(linkText);
                    a.title = "Link To Hire";
                    a.href = element.hireLink;
                    a.target = "_blank";

                    cell.appendChild(a);
                }else if(key2 == "hireLink" || key2 == "rcGroup"){
    ...
    function createShadowTablePartial(){
        ...
        for (let element of shadows_date_partial) {
            let row = shadowTable.insertRow();
            for (let key2 in element) {
                if(key2 == "workStep"){
                    let cell = row.insertCell();
    ...
    function createShadowTableDate(){
        ...
        for (let element of shadows_date) {
            let row = shadowTable.insertRow();
            for (let key2 in element) {
                if(key2 == "workStep"){
                    let cell = row.insertCell();
    ...
    function createNoShadowTable(){
        ...
        let row = thead.insertRow();
        for (let key of data) {
            if(key != "hireLink" && key != "shadow" && key != "id" && key != "rcGroup"){
                let th = document.createElement("th");
                let text = document.createTextNode(key);
                th.appendChild(text);
                row.appendChild(th);
        ...
        for (let element of report) {
            if(element.shadow === false && element.type === "Phone Screen"){
                let row = shadowTable.insertRow();
                for (let key2 in element) {
                    if(key2 == "workStep"){
                        let cell = row.insertCell();
                        ...
                        cell.appendChild(a);
                    }else if(key2 == "hireLink" || key2 == "shadow" || key2 == "id" || key2 == "rcGroup"){
